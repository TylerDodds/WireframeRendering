...
VertexOutput Vertex(Attributes input) 
{
	...
	#if defined(WIREFRAME_NEEDS_OBJECT_NORMALS)
	output.normalVS = TransformWorldToViewDir(GetVertexNormalInputs(input.normalOS).normalWS);
	#endif
	#if defined(WIREFRAME_NEEDS_FACE_NORMALS)
	output.faceNormalVS = TransformWorldToViewDir(TransformObjectToWorldNormal(input.faceNormalOS, false));
	#endif
	return output;
}
...
{
	float3 startVS = inputs[0].positionVS;
	float3 endVS = inputs[1].positionVS;
	...
	#if !defined(_WIREFRAME_CONTOUR_EDGES_NOTIMPORTED)
	float3 faceNormal0VS = inputs[0].faceNormalVS;
	float3 faceNormal1VS = inputs[1].faceNormalVS;
	...
	float startU = -_WireframeCutoff / _Width;
	float endU = 1 - startU;
	#else
	float startU = 0;
	float endU = 1;
	...
	float2 startLeftSS = startSS - segmentNormalSS * widthSS;
	float2 startRightSS = startSS + segmentNormalSS * widthSS;
	float2 endLeftSS = endSS - segmentNormalSS * widthSS;
	float2 endRightSS = endSS + segmentNormalSS * widthSS;
	...
	outputStream.Append(SetupVertexSS(startRightSS, float2(endU, overshootStartV), segmentLengthScaleSS, closestStartVS, closestStartClip));
	outputStream.Append(SetupVertexSS(startLeftSS, float2(startU, overshootStartV), segmentLengthScaleSS, closestStartVS, closestStartClip));
	outputStream.Append(SetupVertexSS(endRightSS, float2(endU, overshootEndV), segmentLengthScaleSS, closestEndVS, closestEndClip));
	outputStream.Append(SetupVertexSS(endLeftSS, float2(startU, overshootEndV), segmentLengthScaleSS, closestEndVS, closestEndClip));
	#else
	float2 startLeftSS = startSS - segmentNormalSS * widthSS;
	float2 startRightSS = startSS + segmentNormalSS * widthSS;
	float2 endLeftSS = endSS - segmentNormalSS * widthSS;
	...
	outputStream.Append(SetupVertexSS(startRightSS, float2(endU, startV), segmentLengthScaleSS, startVS, startClip));
	outputStream.Append(SetupVertexSS(startLeftSS, float2(startU, startV), segmentLengthScaleSS, startVS, startClip));
	outputStream.Append(SetupVertexSS(endRightSS, float2(endU, endV), segmentLengthScaleSS, endVS, endClip));
	outputStream.Append(SetupVertexSS(endLeftSS, float2(startU, endV), segmentLengthScaleSS, endVS, endClip));
	#endif
	...
	float3 normalVS = SafeNormalize((inputs[0].normalVS + inputs[1].normalVS) * 0.5);
	...
	float3 startLeftVS = startVS - binormalVS * widthWs;
	float3 startRightVS = startVS + binormalVS * widthWs;
	float3 endLeftVS = endVS - binormalVS * widthWs;
	float3 endRightVS = endVS + binormalVS * widthWs;
	...
	outputStream.Append(SetupVertexVS(startRightVS, float2(endU, startVWithOvershoot), origSegmentLengthVS, startVS));
	outputStream.Append(SetupVertexVS(startLeftVS, float2(startU, startVWithOvershoot), origSegmentLengthVS, startVS));
	outputStream.Append(SetupVertexVS(endRightVS, float2(endU, endVWithOvershoot), origSegmentLengthVS, endVS));
	outputStream.Append(SetupVertexVS(endLeftVS, float2(startU, endVWithOvershoot), origSegmentLengthVS, endVS));
	#else
	float3 startLeftVS = startVS - binormalVS * widthWs;
	float3 startRightVS = startVS + binormalVS * widthWs;
	float3 endLeftVS = endVS - binormalVS * widthWs;
	...
	outputStream.Append(SetupVertexVS(startRightVS, float2(endU, startV), origSegmentLengthScaleVS, startVS));
	outputStream.Append(SetupVertexVS(startLeftVS, float2(startU, startV), origSegmentLengthScaleVS, startVS));
	outputStream.Append(SetupVertexVS(endRightVS, float2(endU, endV), origSegmentLengthScaleVS, endVS));
	outputStream.Append(SetupVertexVS(endLeftVS, float2(startU, endV), origSegmentLengthScaleVS, endVS));
	#endif
